...
export default async function AdminPracticePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-8 px-6 py-10">
      <h1 className="text-3xl font-semibold">Manage sections</h1>
      <p className="text-sm text-[var(--color-text-muted)]">
        Practice sets group problems. Create at least one skill and one level before adding a problem inside a section.
      </p>
            ...
            {sets.map((set) => (
              <tr key={set.id} className="border-t border-[var(--color-border)]">
                ...
                <td className="px-4 py-3">
                  <a className="text-[var(--color-primary)]" href={`/admin/practice/${set.id}`}>
                    {set.title}
                  </a>
                </td>
                ...
                <td className="flex gap-2 px-4 py-3">
                  <form action={movePracticeSetUp}>
                    <input type="hidden" name="setId" value={set.id} />
                    <button type="submit">↑</button>
                  </form>
                  <form action={movePracticeSetDown}>
                    <input type="hidden" name="setId" value={set.id} />
                    <button type="submit">↓</button>
                  </form>
